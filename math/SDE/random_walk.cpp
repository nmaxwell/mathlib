...
void gen_BM( double time_step, int n_steps, double *& W, int32 * walk, int32 * pool )
{
    ...
    int_to_random_walk ( pool, n_steps, walk );
    
    for ( int k=0; k<n_steps; k++ )
        W[k] = time_step*walk[k]*32;
}
...
void gen_BM( double time_step, int n_steps, double *& W )
{
    ...
    int32 *walk = ml_alloc<int32 > (n_steps+1);
    int32 *pool = ml_alloc<int32 > (n_steps);
    
    gen_BM( time_step, n_steps, W, walk, pool);
...
void gen_BM( double time_step, int n_steps, double **& W, int n_runs )
{
    ...
    for (int j=0; j<n_runs; j++)
        gen_BM( time_step, n_steps, W[j], walk, pool);
